...
- `cloudflared/`: Cloudflare Tunnel 起動ラッパー
...
手順:
1. balenaCloud の fleet または device の環境変数に、少なくとも `MARIADB_PASSWORD`、`MARIADB_ROOT_PASSWORD`、`MYSQL_PASSWORD`、`MYSQL_ROOT_PASSWORD`、`GF_SECURITY_ADMIN_PASSWORD` を登録します。Cloudflare Tunnel を使う場合は `CF_TUNNEL_TOKEN` も登録します。
...
2. `balena login` または `balena login --token <API_TOKEN>` でログインします。
3. `balena push Pi-Smarthome --source .` を実行してデプロイします。

balenaCloud 側の環境変数を優先する構成です。機密値は Git に含めないでください。
...
- GitHub Actions では Node 24 を使い、`balena-cli` の要求バージョンに合わせています。
...
- USB が未接続または mount に失敗した場合は終了せず、待機して再試行する
...
## Cloudflare Tunnel
Cloudflare の公式手順では、リモート管理トンネルは token だけで実行できます。Docker 実行例も `cloudflare/cloudflared:latest tunnel --no-autoupdate run --token <TUNNEL_TOKEN>` です。このリポジトリでは balena 上で確実に同じ引数を渡せるよう、`cloudflared/entrypoint.sh` を使う小さなラッパーイメージで起動しています。
