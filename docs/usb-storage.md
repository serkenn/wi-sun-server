...
ただし、USB を mount するだけでは不十分です。SD 消耗対策として重要なのは、live data が実際に USB 側へ書かれることです。
...
- `usb-storage` サービスは USB の自動 mount を補助しますが、MariaDB や Grafana の named volume 実体をそのまま USB に移す機能ではありません。
- したがって、本気で SD 消耗を避けるなら `/var/lib/docker` を含む Docker 永続データの扱いを USB 前提で見直す必要があります。
